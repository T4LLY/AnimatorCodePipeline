...
さらに、Avatar内に既存の`AnimatorCodePipelineSettings`が存在するか、そのSettingsにどのModuleが直接登録されているか、同じGameObjectに`ModularAvatarMergeAnimator`が存在するか、MA Merge Animatorに通常のAnimator ControllerとどのPlayable Layer・Path Modeが設定されているか、既存のModuleに同じ機能や関連するParameterが存在しないかなどを確認し、既存の構成を再利用できる場合はそれを利用します。Animator ControllerとAnimator統合設定はMA Merge AnimatorをSource of Truthとし、Animator Code Pipelineへ重複して保持しません。

必要なAnimator Code Pipelineの構成がまだ存在しない場合は、現在のAvatar構成に合わせて必要なAnimator Code Pipelineと同じホスト上のModular Avatar Merge Animatorを用意し、MA Merge Animatorへプロジェクト所有の通常のAnimator Controller、Playable Layer、Path設定を構成します。ModuleはACP Settingsへ直接追加します。ACPはMA Merge AnimatorのControllerをビルド中だけcloneし、元のControllerやアバター本体のFX Controllerは直接変更しません。
...
作成したC# Moduleは対象の`AnimatorCodePipelineSettings`へ直接登録し、そのSettingsから実際に呼び出されることを確認します。複数のSettingsが存在する場合も、単に新しいものを追加するのではなく、現在のAvatar構成を確認して適切なSettingsへ追加します。
